...
from model_randla import RandLANet
import time
#from network_blocks import get_block
#from KPCNN import KPCNN
#from config import Config
#from pointnet2.models.pointnet2_msg_sem.py import PointNet2SemSegMSG
"""
...
class PointGenCon(nn.Module):
    ...
    def forward(self, x):
        ...
        return x
class PointGenCon_RIG(nn.Module):
    def __init__(self, bottleneck_size = 2500):
        self.bottleneck_size = bottleneck_size
        super(PointGenCon_RIG, self).__init__()
        self.conv1 = torch.nn.Conv1d(self.bottleneck_size, self.bottleneck_size, 1)
        self.conv2 = torch.nn.Conv1d(self.bottleneck_size, self.bottleneck_size//2, 1)
        self.conv3 = torch.nn.Conv1d(self.bottleneck_size//2, self.bottleneck_size//4, 1)
        self.conv4 = torch.nn.Conv1d(self.bottleneck_size//4, 7, 1)
    ...
    def forward(self, x):
        ...
        x = 2*self.th(self.conv4(x))
        return x
class GenPointsFeatures(nn.Module):
    def __init__(self, num_points = 6890, bottleneck_size = 1024):
        super(GenPointsFeatures, self).__init__()
        self.num_points = num_points
        self.bottleneck_size = bottleneck_size
        self.encoder = nn.Sequential(
        RandLANet(3,1024,16,4)
        )
        self.conv1 = torch.nn.Conv1d(self.bottleneck_size+self.bottleneck_size, self.bottleneck_size, 1)
        self.conv2 = torch.nn.Conv1d(self.bottleneck_size, self.bottleneck_size // 2, 1)
        self.conv3 = torch.nn.Conv1d(self.bottleneck_size // 2, self.bottleneck_size // 4, 1)
        self.conv4 = torch.nn.Conv1d(self.bottleneck_size // 4, 1, 1)
        self.st = nn.Softmax(dim=2)
        self.bn1 = torch.nn.BatchNorm1d(self.bottleneck_size)
        self.bn2 = torch.nn.BatchNorm1d(self.bottleneck_size // 2)
        self.bn3 = torch.nn.BatchNorm1d(self.bottleneck_size // 4)
    def foward(self,x,y,idx):#输入x和y，idx是随机指数，生成结果在-1到1之间
        x = self.encoder(x)
        x=torch.index_select(x, dim=2, index=idx)
        y = self.encoder(y)
        y = torch.index_select(y, dim=2, index=idx)
        re=self.decoder(x,y)
        return re
    def decoder(self,x,y):#x n 1 f   y n 1 f
        #x=x.permute(0, 2, 1)
        #y=y.permute(0, 2, 1)
        input=torch.cat([x,y],dim=1)
        input=self.conv1(input)
        input = F.relu(self.bn1(input))
        input = F.relu(self.bn2(self.conv2(input)))
        input = F.relu(self.bn3(self.conv3(input)))
        input = F.softmax(self.conv4(input),dim=2)
        return input
...
class AE_AtlasNet_Humans(nn.Module):
    def __init__(self, num_points = 6890, bottleneck_size = 1024, nb_primitives = 1):
        ...
        self.decoder = nn.ModuleList([PointGenCon(bottleneck_size = 3 +self.bottleneck_size),PointGenCon_RIG(bottleneck_size = 3 +self.bottleneck_size)])
    ...
    def forward_rig(self,x):
        x = self.encoder(x)# b f n  8 1024 6890
        #print("X shape is",x.shape)
        outs = []
        rand_grid = self.vertex.transpose(0, 1).contiguous().unsqueeze(0).expand(x.size(0), 3, -1)
        rand_grid = Variable(rand_grid) # b p n 8 3 6890
        y = x.unsqueeze(2).expand(x.size(0), x.size(1), rand_grid.size(2)).contiguous()
        y = torch.cat((rand_grid, y), 1).contiguous()
        outs.append(self.decoder[1](y))
