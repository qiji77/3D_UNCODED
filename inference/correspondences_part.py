...
class Inference(object):
    def __init__(self, HR=0, nepoch=3000, model_path='trained_models/sup_human_network_last.pth', num_points=6890,
        ...
        # load network
        self.network = model_mlp.AE_AtlasNet_Humans(num_points=self.num_points)
        self.network.cuda()
        ...
        self.save_path ="./results_mlp_norig" #save_path
    ...
    def Tran_points_decode(self, input_data,template_riged):
        ...
        rand_grid = rand_grid.view(template_riged.size(0), -1, 3).transpose(2,
                                                                    1).contiguous() 
        #rand_grid = rand_grid.transpose(0, 1).contiguous().expand(input_data.size(0), 3, -1)
        # print("rand_grid test is",rand_grid)
        Rota_point = rand_grid
        Rota_point_t = Rota_point.transpose(2, 1).cuda()
        Cos_theat = torch.cos(Point_theat)
    ...
    def compute_correspondances(self, source_p, source_reconstructed_p, target_p, target_reconstructed_p, path):
        ...
        with torch.no_grad():
            ...
            # save output
            if path is None:
                np.savetxt("results_mlp_norig/correspondences.txt", closest_points, fmt='%1.10f')
            else:
                print("Path is",path)
            ...
            mesh.export("results_mlp_norig/correspondences.ply")
            ...
            for line in f.readlines():
                ...
                if self.save_path is None:
                    self.compute_correspondances(inputA, inputA[:-4] + "FinalReconstruction.ply", inputB,
                                                 ...
                                                 "results_mlp_norig/" + line[:3] + "_" + line[4:7] + ".txt")
                else:
    ...
    def regress(self, points):
        ...
        #latent_code=self.MLP[1](latent_code)
        ...
        while np.log(loss) > -9 and i < self.nepoch:
            ...
            pointsReconstructed =self.Tran_points_decode(input_param,pointsReconstructed_rig)#self.network.decode(input_param) #self.Tran_points_decode(input_param)  # forward pass
        ...
        with torch.no_grad():
            if self.HR:
                pointsReconstructed_rig = self.Tran_points_decode_rig(input_param)
                pointsReconstructed =self.Tran_points_decode_full(input_param,pointsReconstructed_rig)  # forward pass#self.network.decode_full(input_param)
            else:
                pointsReconstructed_rig = self.Tran_points_decode_rig(input_param)
                pointsReconstructed =self.Tran_points_decode(input_param,pointsReconstructed_rig)  # forward pass self.network.decode(input_param)#self.Tran_points_decode(input_param)  # forward pass
