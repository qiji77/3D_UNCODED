...
def Tran_points(Translate_matrix,template_points_toTra,train_sig):
  #  start=time.time()
    ...
    #print("pointtran size",pointtran.size())
  #  end=time.time()
   # print("network time is ",end-start)
    #pointtran_t=pointtran.view(points.size(0), -1, 8)
...
for epoch in range(opt.nepoch):
    ...
    if epoch<0:
        if epoch == 0:
            ...
            for i, data in enumerate(dataloader, 0):
                ...
                pointsReconstructed = Tran_points_rig(points_tra,template_points,train_sig=1,Rig_list=Rig_list)  # forward pass
        ...
        if epoch == 0:
            ...
            for i, data in enumerate(dataloader, 0):
                ...
                pointsReconstructed = Tran_points(points_tra,template_points,train_sig=1)  # 非刚性变换
                #pointsReconstructed =network(points)
                loss_net =  torch.mean((pointsReconstructed - template_points)**2)
                loss_net.backward()
                optimizer.step()  # gradient update
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            pointsReconstructed = Tran_points(points_tra, template_points, train_sig=1)
            #compute the laplacian loss
            ...
            if epoch >2:
                pointsReconstructed_clone=pointsReconstructed.clone()
                pointsReconstructed_clone=pointsReconstructed_clone.reshape(pointsReconstructed.size(0)*pointsReconstructed.size(1),pointsReconstructed.size(2))
                #print("New pointsReconstructed size is",pointsReconstructed_clone.size())
                points_clone = points.transpose(2, 1).clone()
                #print("Point sieze is ", points.size())
                points_clone = points_clone.reshape(points.size(0) * points.size(2), points.size(1))
                #print("New points size is", points_clone.size())
                neigh.fit(points_clone.cpu().detach().numpy())

                dis,idx_knn = neigh.kneighbors(pointsReconstructed_clone.cpu().detach().numpy(), return_distance=True)

                #print(idx_knn)
                #print("Distance is",dis)
                # correspondances throught template
                closest_points = points_clone[idx_knn]
                #closest_points = np.mean(closest_points, 1, keepdims=False)
                #print("closest_points size is ",closest_points.size())
                closest_points=closest_points.reshape(pointsReconstructed.size(0),pointsReconstructed.size(1),3)
                closest_points=closest_points.transpose(2, 1)
                recon_loss = Tran_loss(points_tra, points, pointsReconstructed_RIG)
                #print("recon_loss is ",recon_loss)

                loss_net = (torch.mean(dist1)) + (torch.mean(dist2)) + lambda_laplace * regul + lambda_ratio* compute_score(pointsReconstructed, network.mesh.faces, target)+10*recon_loss
            else:
                loss_net = (torch.mean(dist1)) + (
                    torch.mean(dist2)) + lambda_laplace * regul + lambda_ratio * compute_score(pointsReconstructed,
                                                                                               network.mesh.faces, target)
            loss_net.backward()
            train_loss_L2_smpl.update(loss_net.item())
            optimizer.step()  # gradient update
            # VIZUALIZE
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(dataloader_smpl_test, 0):
            ...
            pointsReconstructed = Tran_points(points_tra, points, train_sig=1)
            #points_tra = network.forward(points)
            #pointsReconstructed = network(points)
