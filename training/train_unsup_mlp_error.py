...
def Tran_loss(Translate_matrix, template_points_toTra, Target_points):
    #idx_c2 = idx.clone()
    # idx_e2 = idx_c2.unsqueeze(-1).expand(opt.batchSize, 6890, 3).cuda()
    #Translate_matrix=Translate_matrix.transpose(2,1).contiguous()
   # Translate_matrix=Translate_matrix.gather(1, idx_e2)
   # Translate_matrix = Translate_matrix.transpose(2, 1).contiguous()
    ...
    if epoch < 30:
        if epoch == 0:
            ...
            for i, data in enumerate(dataloader, 0):
                ...
                print('init [%d: %d/%d] train loss:  %f' % (epoch, i, len_dataset / opt.batchSize, loss_net.item()))
        for i, data in enumerate(dataloader, 0):
            optimizer.zero_grad()
            points, idx,_, _ = data
            points = points.transpose(2, 1).contiguous()
    ...
    if epoch%5==0:
        with torch.no_grad():
            ...
            for i, data in enumerate(dataloader_smpl_test, 0):
                ...
                print('[%d: %d/%d] test smlp loss:  %f' % (epoch, i, len_dataset / opt.batchSize, loss_net.item()))
