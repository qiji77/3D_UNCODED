...
def Tran_loss(Translate_matrix,template_points_toTra,Target_points,idx):
    Recon_points=Tran_points(Translate_matrix,template_points_toTra,None,idx)
    idx_c2 = idx.clone()
    idx_e2 = idx_c2.unsqueeze(-1).expand(32, 6890, 3).cuda()

    loss=torch.mean((Recon_points - Target_points.gather(1,idx_e2))**2)#.gather(1,idx_e2)
    return loss
def Tran_points(Translate_matrix,template_points_toTra,train_sig,idx=None):
...
for epoch in range(opt.nepoch):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        if epoch >40:
            recon_loss = Tran_loss(points_tra, points, template_points,idx)#输入（旋转参数，被旋转点，loss的目标点,真实值坐标）
            #print("recon_loss is ",recon_loss)

            loss_net = (torch.mean(dist1)) + (torch.mean(dist2)) + lambda_laplace * regul + lambda_ratio* compute_score(pointsReconstructed, network.mesh.faces, target)+recon_loss
        else:
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(dataloader_smpl_test, 0):
            ...
            points_tra = network.forward(points)
            pointsReconstructed = Tran_points(points_tra, template_points_temp, train_sig=1)
