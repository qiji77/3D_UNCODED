    ...
    if epoch<0:
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            if epoch >7:
                ...
                recon_loss = Tran_loss(points_tra, closest_points, pointsReconstructed_RIG)'''
                #pointsReconstructed_clone = pointsReconstructed.clone()
                #neigh = NearestNeighbors(n_neighbors=1, radius=1.0, algorithm='auto', leaf_size=50, metric='minkowski', p=2, metric_params=None, n_jobs=-1)
                points_clone = points.transpose(2, 1)
                points_close = points_clone
                knn = KNN(k=1, transpose_mode=True)
                dist, indx = knn(pointsReconstructed, points_close)                           
                ...
                print("dist1 is",torch.mean(dist1))
                print("dist2 is",torch.mean(dist2))
                print("regul is",regul)
                
                score=compute_score(pointsReconstructed,network.mesh.faces, target)
                print("score is",score)
                print("recon_loss is",recon_loss)
                loss_net = (torch.mean(dist1)) + (torch.mean(dist2)) + lambda_laplace * regul + lambda_ratio*score+5*recon_loss
            else:
                print("dist1 is",torch.mean(dist1))
                print("dist2 is",torch.mean(dist2))
                print("regul is",regul)
                ...
                print("score is",score)
                loss_net = (torch.mean(dist1)) + (
                    torch.mean(dist2)) + lambda_laplace * regul + lambda_ratio * score
            loss_net.backward()
            train_loss_L2_smpl.update(loss_net.item())
            optimizer.step()  # gradient update
            print('[%d: %d/%d] train loss:  %f' % (epoch, i, len_dataset / 32,  loss_net.item()))
